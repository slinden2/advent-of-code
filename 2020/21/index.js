const solve1 = (input) => {
  ...
  input.forEach((row) => {
    const [ingredients, allergens] = row
      .replace(")", "")
      .split(" (contains ")
      .map((x) => x.split(/[\s,]+/g));
    ...
    for (const a of allergens) {
      if (a in allergensToIngredients) {
        allergensToIngredients[a] = new Set(
          ingredients.filter((i) => allergensToIngredients[a].has(i))
        );
      } else {
  ...
  const safeIngredients = [...allIngredients].filter(
    (ingredient) => !(ingredient in definedIngredients)
  );

  const count = safeIngredients
    .map((ingredient) => ingredientCounter[ingredient])
