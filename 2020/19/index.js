const getParsedData = (input) => {
  ...
  input[0].split("\n").forEach((r) => {
    const { groups } = /^(?<key>\d+): (?<value>.*)$/.exec(r);
...
const ruleToRegex = {};
const computeRules = (value, rules) => {
  if (value in ruleToRegex) {
    return ruleToRegex[value];
  }
  ...
  ruleToRegex[value] = result;
  return result;
};

const solve1 = (input) => {
  const [rules, messages] = getParsedData(input);

  computeRules(rules[0], rules);

  const mainRule = new RegExp(`^${ruleToRegex[rules[0]]}$`);
  console.log(mainRule);

  let sum = 0;
  messages.forEach((msg) => {
    if (mainRule.test(msg)) sum++;
  });
...
const solve2 = (input) => {
  ...
  console.log(ruleToRegex);

  const rule = new RegExp(
    "^(?<group42>(" +
      ruleToRegex[rules[42]] +
      ")+)(?<group31>(" +
      ruleToRegex[rules[31]] +
      ")+)$"
  );
  ...
  for (const message of messages) {
    ...
    if (matches) {
      ...
      const matches42 = groups.group42.match(
        new RegExp(ruleToRegex[rules[42]], "g")
      ).length;
      const matches31 = groups.group31.match(
        new RegExp(ruleToRegex[rules[31]], "g")
      ).length;
...
const inputExample2 = `42: 9 14 | 10 1
9: 14 27 | 1 26
10: 23 14 | 28 1
1: "a"
11: 42 31
...
bbbabbbbabbabbababababbabbbaaaaaabaabbabbaabbabb`.split("\n\n");

// solve1(inputExample1);
// solve1(input);
